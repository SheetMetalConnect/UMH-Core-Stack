...
│                                                                             │
│  Host IP:80    ──► NGINX ──► umh:8040 (Webhook Gateway)                    │
│  Host IP:443   ──► NGINX (HTTPS)                                          │
...
│  Host IP:3003  ──► Postgres MCP (AI DB Access)                             │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                              ↓ Docker Port Mapping
┌─────────────────────────────────────────────────────────────────────────────┐
...
│                                                                             │
│  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐               │
│  │   umh     │  │  nginx    │  │  hivemq   │  │  nodered  │               │
│  │ :8040     │  │  :8080    │  │  :1883    │  │  :1880    │               │
│  │ :8051     │  │           │  │  :8000    │  │           │               │
│  └───────────┘  └───────────┘  └───────────┘  └───────────┘               │
│                                                                             │
│  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌─────────────┐             │
│  │ portainer │  │  grafana  │  │ pgbouncer │◄── Bridge to DB │             │
│  │ :9000     │  │  :3000    │  │ :5432     │    network     │             │
│  └───────────┘  └───────────┘  └───────────┘  └─────────────┘             │
│                                                                             │
│  ┌─────────────┐                                                           │
│  │ postgres-mcp│                                                           │
...
│                                                                             │
│  ┌───────────────┐                                                         │
│  │  timescaledb  │  ← Not accessible from default network or host directly │
│  │  :5432        │                                                         │
│  └───────────────┘                                                         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
The stack uses two Docker networks for security isolation. UMH Core runs inside the compose stack and is attached to the application network so NGINX, HiveMQ, and PgBouncer can reach it.
...
| postgres-mcp | Yes | AI/LLM read-only database access |
| umh | Yes | Edge gateway with embedded Kafka (Redpanda) |
...
|---------|---------------|-----------|---------------------|---------|
| NGINX (Webhooks) | 80 | `PORT_HTTP` | `PORT_HTTP` | 80 |
| NGINX (HTTPS) | 443 | `PORT_HTTPS` | `PORT_HTTPS` | 443 |
...
| Postgres MCP | 3003 | `MCP_POSTGRES_PORT` | `MCP_POSTGRES_PORT` | 3003 |
...
|---------|--------------|---------|
| umh | 8040 | HTTP/Webhook input (proxied via NGINX) |
| umh | 8051 | Management/GraphQL endpoints |
...
server {
    ...
    location /webhook/ {
        proxy_pass http://umh:8040;
    }
}
```

The file is located at `nginx.conf` in the repo root and mounted to
`/etc/nginx/conf.d/default.conf` in the container. If you add TLS
certificates, mount them at `./certs` and update the HTTPS server block.

### SSL/TLS Termination

When NGINX handles SSL/TLS termination (HTTPS), umh needs to accept forwarded requests from the proxy. Add this environment variable to umh:
...
This tells umh to trust the `X-Forwarded-Proto` header from NGINX, allowing it to properly handle requests that were originally HTTPS but arrive as HTTP after proxy termination.

**When to use:**
- NGINX terminates SSL (listens on 443 with certificates)
- Requests are forwarded to umh over HTTP internally
- You need umh to recognize the original protocol was HTTPS
...
3. Data enters Unified Namespace
   ...
   Example: umh.v1.factory1/my/sensor/topic
```
...
```bash
psql -h <host-ip> -p 5432 -U grafanareader -d umh
```
...
- Inspect networks (<project>_default, timescaledb-network)
...
- `<project>_default`: Main service network
...
| TimescaleDB | `pg_isready -U postgres -d umh` | 10s |
...
```bash
# Port Configuration
PORT_HTTP=80              # NGINX (HTTP)
PORT_HTTPS=443            # NGINX (HTTPS)
...
MCP_POSTGRES_PORT=3003    # Postgres MCP
```
...
| PgBouncer writer | kafkatopostgresqlv2 | changeme |
| PgBouncer reader | grafanareader | changeme |
...
```bash
docker logs umh --tail 50
```

## See Also

- [Operations](operations.md) - Quick start and commands
- [Historian](historian.md) - TimescaleDB addon details
- [Integration Patterns](integration-patterns.md) - ERP patterns
